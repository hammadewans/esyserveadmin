...
(async function fetchSchoolCount() {
  try {
    const response = await fetch('http://localhost/count/school', {
      method: 'GET',
      credentials: 'include'
    });
...
(async function fetchSchoolCount() {
  try {
    const response = await fetch('http://localhost/count/student', {
      method: 'GET',
      credentials: 'include'
    });
...
(async function fetchSchoolCount() {
  try {
    const response = await fetch('http://localhost/count/teacher', {
      method: 'GET',
      credentials: 'include'
    });
...
(async function fetchBlockedUsers() {
  try {
    const response = await fetch('http://localhost/user/blocked', {
      method: 'GET',
      credentials: 'include'
    });
...
document.addEventListener('click', async function (event) {
  ...
  try {
    const response = await fetch(`http://localhost/user/reactivate/${userId}`, {
      method: 'POST',
      credentials: 'include'
    });
    if (response.status === 401) return window.location.href = 'login.html';
