(async function fetchTemplates() {
  try {
    const response = await fetch('http://localhost/fetch/template', {
      credentials: 'include',
      method: 'GET'
    });
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  async function fetchTeachers(userid) {
    try {
      const response = await fetch(`http://localhost/fetch/teacher/${userid}`, {
        credentials: 'include',
        method: 'GET'
      });
  ...
  document.getElementById('sendSelectedTeachers').addEventListener('click', async () => {
    ...
    try {
      const response = await fetch('http://localhost/teacher/pdf', {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
