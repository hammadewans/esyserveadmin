  ...
  document.getElementById('sendSelected').addEventListener('click', async () => {
    ...
    win.document.write(`  
      <html>  
        <head>  
          <style>  
            .row { display: flex; justify-content: center; margin-bottom: 0; }  
    ...
    async function loadBatch() {  
      ...
      try {  
        ...
        // 2 cards per row, centered, no vertical spacing
        let html = '';  
    ...
    win.addEventListener('scroll', () => {  
      const nearBottom = win.innerHeight + win.scrollY >= win.document.body.offsetHeight - 200;  
    ...
    async function loadBatch() {  
      ...
      try {  
        ...
        for (let i = 0; i < result.length; i += 2) {  
          ...
          const extraMargin = (rowIndex + 1) % 5 === 0 ? 'margin-bottom:40px;' : 'margin-bottom:10px;';  

          html += `<div class="row" style="${extraMargin}">`;  
          for (let j = i; j < i + 2 && j < result.length; j++) {  
            html += `<div class="card">${result[j]}</div>`;  
          }  
          html += '</div>';  
